...
test.describe('Notifications UI SSO login and landing', () => {
  test('landing or dashboard loads', async ({ page }) => {
    await page.goto('/');
    const signInOrContent = page.getByRole('link', { name: /sign in|login/i }).or(page.getByText(/dashboard|notifications/i));
  ...
  test('full SSO login then authenticated indicator', async ({ page }) => {
    ...
    if (onAccounts) {
      ...
      await page.waitForURL(/notifications\.codevertexitsolutions\.com|localhost/, { timeout: 25_000 }).catch(() => {});
    }
    const dashboardOrContent = page.getByRole('link', { name: /dashboard|profile/i }).or(page.getByText(/dashboard|notifications/i));
