...
func (s *Service) GetUserRoles(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*NotificationRole, error) {
	return s.repo.GetUserRoles(ctx, tenantID, userID)
}

// GetUserPermissions retrieves all permissions for a user.
func (s *Service) GetUserPermissions(ctx context.Context, tenantID uuid.UUID, userID uuid.UUID) ([]*NotificationPermission, error) {
	return s.repo.GetUserPermissions(ctx, tenantID, userID)
}

// ListRoles lists all roles for a tenant.
func (s *Service) ListRoles(ctx context.Context, tenantID uuid.UUID) ([]*NotificationRole, error) {
	return s.repo.ListRoles(ctx, tenantID)
}

// ListPermissions lists all permissions.
func (s *Service) ListPermissions(ctx context.Context, filters PermissionFilters) ([]*NotificationPermission, error) {
	return s.repo.ListPermissions(ctx, filters)
}

// ListUserAssignments lists role assignments.
func (s *Service) ListUserAssignments(ctx context.Context, tenantID uuid.UUID, filters AssignmentFilters) ([]*UserRoleAssignment, error) {
	return s.repo.ListUserAssignments(ctx, tenantID, filters)
}
