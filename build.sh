...
if ! kubectl -n "$NAMESPACE" get secret "$ENV_SECRET_NAME" >/dev/null 2>&1; then
  ...
  kubectl -n "$NAMESPACE" create secret generic "$ENV_SECRET_NAME" \
    --from-literal=NOTIFICATIONS_POSTGRES_URL="postgres://postgres:postgres@postgresql.notifications.svc.cluster.local:5432/notifications?sslmode=disable" \
    --from-literal=NOTIFICATIONS_REDIS_ADDR="redis-master.notifications.svc.cluster.local:6381" \
    --from-literal=NOTIFICATIONS_NATS_URL="nats://nats.messaging.svc.cluster.local:4222" \
    --from-literal=NOTIFICATIONS_DEFAULT_EMAIL_SENDER="Urban Cafe <hello@bengobox.com>" || true
fi
