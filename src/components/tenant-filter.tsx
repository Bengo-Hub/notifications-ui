...
/**
 * TenantFilter — single-select dropdown shown only to platform owners, in the top nav (see
 * header.tsx). Selecting a tenant scopes every tenant-scoped page/request to that tenant's data
 * (see client.ts's request interceptor); "All Tenants" (the default, no selection) is
 * platform-wide. Ported from subscriptions-ui's identical component/store pair so both apps give
 * a platform admin the same "act as tenant X" mechanism.
 */
export function TenantFilter({ className }: { className?: string }) {
    const user = useAuthStore((s) => s.user);
    const isPlatformOwner = (user as any)?.isPlatformOwner || (user as any)?.tenantSlug === 'codevertex';
    ...
    const { data: tenants = [] } = useQuery({
        ...
        queryFn: async () => (await settingsApi.listPlatformTenants()).filter((t) => t.slug !== 'codevertex'),
    ...
    const select = (t: { id: string; name: string; slug: string }) => {
